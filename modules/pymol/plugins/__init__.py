...
def set_startup_path(p, autosave=True):
    if isinstance(p, list):
        ...
        print(_tr('PluginManager', ' Error: set_startup_path failed'))
...
def pref_save(filename=PYMOLPLUGINSRC, quiet=1):
    ...
    try:
        ...
        print(_tr('PluginManager', ' Plugin-Error: Cannot write Plugins resource file to'), filename)
        return
    ...
    if not int(quiet):
        print(_tr('PluginManager', ' Plugin settings saved!'))
...
def addmenuitem(label, command=None, menuName='Plugin'):
    ...
    if pmgapp is not None:
        for i in range(1, len(labels2)):
            try:
                pmgapp.menuBar.addcascademenu(labels2[i-1], labels2[i], label=_mtr(labels1[i]))
            except ValueError:
                pass
        if labels1[-1] == '-':
            pmgapp.menuBar.addmenuitem(labels2[-1], 'separator')
        else:
            pmgapp.menuBar.addmenuitem(labels2[-1], 'command', label=_mtr(labels1[-1]),
                    command=command)
...
DESCRIPTION
    ...
    if info.loaded:
        if not int(quiet):
            print(_tr('PluginManager', ' info: plugin already loaded'))
        return
    info.load()
...
class PluginInfo(object):
    ...
    def get_docstring(self):
        ...
        try:
            ...
            return _tr('PluginManager', 'WARNING: Plugin not Python 3.x compatible: ') + str(e)
        except:
            pass
    ...
    def load(self, pmgapp=None, force=0):
        ...
        try:
            ...
            if verbose and pymol.invocation.options.show_splash:
                print(_tr('PluginManager', ' Plugin "%s" loaded in %.2f seconds') % (self.name, self.loadtime))
        except QtNotAvailableError:
            colorprinting.warning(_tr('PluginManager', "Plugin '%s' only available with PyQt GUI.") % (self.name,))
        except:
            e = sys.exc_info()[1]
            if verbose:
                colorprinting.print_exc([__file__])
            elif 'libX11' in str(e) and sys.platform == 'darwin':
                colorprinting.error(_tr('PluginManager', 'Please install XQuartz (https://www.xquartz.org/)'))
            else:
                colorprinting.error(e)
            colorprinting.warning(_tr('PluginManager', "Unable to initialize plugin '%s' (%s).") % (self.name, self.mod_name))
            return False
    ...
    def uninstall(self, parent=None):
        ...
        ok = tkMessageBox.askyesno(_tr('PluginManager', 'Confirm'),
                _tr('PluginManager', 'Do you really want to uninstall plugin "%s"') % (self.name), parent=parent)
        if not ok:
            return False
        ...
        try:
            ...
            showinfo(_tr('PluginManager', 'Error'), _tr('PluginManager', 'Could not delete files for plugin "%s".') % (self.name), parent=parent)
            return False
        ...
        showinfo(_tr('PluginManager', 'Info'), _tr('PluginManager', 'Plugin "%s" successfully removed. Please restart PyMOL.') % (self.name), parent=parent)
        return True
...
def findPlugins(paths):
            ...
            if '.' in filename:
                ...
                if ext == 'py':
                    if name not in modules:
                        ...
                        print(_tr('PluginManager', ' warning: multiple plugins named'), name)
            else:
    ...
    if verbose:
        print(_tr('PluginManager', ' Scanning for modules took %.4f seconds') % (time.time() - start))
    return modules
